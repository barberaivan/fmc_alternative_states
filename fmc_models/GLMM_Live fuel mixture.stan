...
data{
  ...
  matrix[N, N_points] point_matrix;
...
// for mu -----------------------------------------------------------------
  ...
  // point effect
  vector[N_points] mu_error_points_raw;
  real<lower = 0> mu_sigma_points_raw;  
...
transformed parameters{
  ...
  real mu_lambda = mu_lambda_raw * prior_mu_lambda_sd;
  ...
  // point effect
  real<lower = 0> mu_sigma_points = mu_sigma_points_raw * prior_mu_sigma_points_sd;  
  vector[N_points] mu_error_points = mu_error_points_raw * mu_sigma_points;
  ...
  // date * site effect 
  real<lower = 0> phi_sigma_ds = phi_sigma_ds_raw * prior_phi_sigma_ds_sd; // variances between dates for every site
  matrix[N_dates, N_sites] phi_error_ds = phi_error_ds_raw * phi_sigma_ds;
...
model{
  ...
  mu_error_points_raw ~ std_normal();
  
  mu_sigma_sites_raw ~ std_normal();
  mu_sigma_ds_raw ~ std_normal();
  mu_sigma_points_raw ~ std_normal();
  ...
  for(n in 1:N)
    y[n] ~ gamma(1 / phi[n], 1 / (mu[n] * phi[n]));
}
...
generated quantities {
  ...
  /* 
    ...
    (and point) normal random effect within sites, at the log scale.
  ...
  */
  ...
  // Date-site means marginal to point effect.
  vector[N_unique] ds_means;
  ...
  vector[N_sites] mu_sigma_ds2 = mu_sigma_ds .* mu_sigma_ds;
    ...
    // compute prediction.
  vector[N_pred] pred_mu = exp(mu_eta + vpd_pred * mu_lambda + 
                               0.5 * (mu_sigma_sites ^ 2 + 
                                      mean(mu_sigma_ds2) +
                                      mu_sigma_points ^ 2)
                               );
  real pred_phi = exp(phi_eta + 
                      0.5 * (phi_sigma_sites ^ 2 + phi_sigma_ds ^ 2));
  
  // Fill mu and phi by site
  for(s in 1:N_sites) {
    sites_mu[s] = exp(mu_eta + mu_error_sites[s] + 
                      0.5 * (mu_sigma_ds[s] ^ 2 + mu_sigma_points ^ 2));
    sites_phi[s] = exp(phi_eta + phi_error_sites[s] + 0.5 * phi_sigma_ds ^ 2);
  }

  // Compute date-site means
  ds_means = exp(mu_eta + 
                 site_matrix[unique_id, ] * mu_error_sites +
                 ds_matrix[unique_id, ] * to_vector(mu_error_ds) +
                 0.5 * mu_sigma_points ^ 2);
}
